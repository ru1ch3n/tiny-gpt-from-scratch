...
def softmax_overflow_demo(large_value):
    ...
    return {"naive_exp":exp,
            "overflowed":np.isinf(exp),}
    pass
...
def read_text_file(text_blob):
    ...
    if not isinstance(text_blob, str):
        raise TypeError("text_blob must be str")
    if text_blob == "":
...
def encode_corpus_to_int_array(text, stoi):
    ...
    return np.array([ stoi[char] for char in text])
    pass
...
def pick_block_size(default_size):
    ...
    return default_size if default_size > 0 else 1
    pass
...
def slice_x_at_offset(data, i, block_size):
    ...
    return data[i:i + pick_block_size(block_size)]
    pass
...
def slice_y_at_offset(data, i, block_size):
    ...
    return data[i + 1: i + 1 + pick_block_size(block_size)]
    pass
...
def sample_next_token(p_matrix, current_id, rng):
    ...
    p = p_matrix[current_id]
    v_size = len(p)
    next_id = rng.choice(v_size, p = p)
    return next_id
    pass
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    next_id = np.zeros(length, dtype = np.int64)
    next_id[0] = start_id
    for i in range(1, length):
        next_id[i] = sample_next_token(p_matrix, next_id[i-1], rng)
    return next_id
    pass
...
def sum_negative_log_probs(p_matrix, data):
    ...
    Y = data[1:]
    for x, y in zip(X, Y):
        res -= log_prob_of_pair(p_matrix, x, y)
    return res
    pass
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    ids = np.asarray(ids)
    N = len(ids)
    one_hot = make_2d_zeros(N, vocab_size)
